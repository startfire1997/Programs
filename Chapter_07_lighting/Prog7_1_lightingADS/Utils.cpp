...
std::string Utils::m_filePath = "";

Utils::Utils() {}

string Utils::readShaderFile(const char *filePath) {
	string content;
	std::string shaderFilePath = m_filePath + filePath;
	ifstream fileStream(shaderFilePath, ios::in);
	if (!fileStream) {
		cout << "readShaderFile: "<< shaderFilePath  << " err !"<< endl;
	}
	string line = "";
...
GLuint Utils::loadCubeMap(const char *mapDir) {
	GLuint textureRef;
	string xp = mapDir; xp = m_filePath + xp + "/xp.jpg";
	string xn = mapDir; xn = m_filePath + xn + "/xn.jpg";
	string yp = mapDir; yp = m_filePath + yp + "/yp.jpg";
	string yn = mapDir; yn = m_filePath + yn + "/yn.jpg";
	string zp = mapDir; zp = m_filePath + zp + "/zp.jpg";
	string zn = mapDir; zn = m_filePath + zn + "/zn.jpg";
...
{	GLuint textureRef;
	std::string imagePath = m_filePath + texImagePath;
	textureRef = SOIL_load_OGL_texture(imagePath.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
	if (textureRef == 0) cout << "didnt find texture file " << imagePath << endl;
...
void Utils::setFilePath(const std::string& filePath)
{
	m_filePath = filePath;
}
